...
final class LeftThighMeasurementPresenter: @MainActor MetricDetailPresenter {
    ...
    var timeSeries: [TimeSeries] {
        ...
        return [TimeSeries(name: "Left Thigh Circumference", data: data)]
    }
    ...
    func onAppear() async {
        entries = Self.leftThighEntries(from: interactor.bodyMeasurements, unit: interactor.lengthUnitPreference)
    }

    func onAddPressed() {
        router.showLogMeasurementView(kind: .leftThigh)
    }
    ...
    private static func leftThighEntries(from entries: [BodyMeasurementEntry], unit: LengthUnitPreference) -> [LeftThighMeasurementEntry] {
        entries
            ...
            .compactMap { entry in
                ...
                return LeftThighMeasurementEntry(
                    id: entry.id,
                    date: entry.date,
                    leftThighCircumference: UnitConversion.convertLength(leftThighCircumference, to: unit)
                )
            }
...
extension CoreRouter {
    func showLeftThighMeasurementView(delegate: LeftThighMeasurementDelegate, themeColor: Color? = nil) {
        router.showScreen(.sheet) { router in
            builder.leftThighMeasurementView(router: router, delegate: delegate, themeColor: themeColor)
        }
    }
}

extension CoreBuilder {
    func leftThighMeasurementView(router: AnyRouter, delegate: LeftThighMeasurementDelegate, themeColor: Color? = nil) -> some View {
        MetricDetailView(
