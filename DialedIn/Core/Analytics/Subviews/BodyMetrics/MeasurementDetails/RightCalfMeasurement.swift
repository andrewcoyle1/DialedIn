...
final class RightCalfMeasurementPresenter: @MainActor MetricDetailPresenter {
    ...
    var timeSeries: [TimeSeries] {
        ...
        return [TimeSeries(name: "Right Calf Circumference", data: data)]
    }
    ...
    func onAppear() async {
        entries = Self.rightCalfEntries(from: interactor.bodyMeasurements, unit: interactor.lengthUnitPreference)
    }
    ...
    private static func rightCalfEntries(from entries: [BodyMeasurementEntry], unit: LengthUnitPreference) -> [RightCalfMeasurementEntry] {
        entries
            ...
            .compactMap { entry in
                ...
                return RightCalfMeasurementEntry(
                    id: entry.id,
                    date: entry.date,
                    rightCalfCircumference: UnitConversion.convertLength(rightCalfCircumference, to: unit)
                )
            }
...
extension CoreRouter {
    func showRightCalfMeasurementView(delegate: RightCalfMeasurementDelegate, themeColor: Color? = nil) {
        router.showScreen(.sheet) { router in
            builder.rightCalfMeasurementView(router: router, delegate: delegate, themeColor: themeColor)
        }
    }
}
