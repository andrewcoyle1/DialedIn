...
final class RightAnkleMeasurementPresenter: @MainActor MetricDetailPresenter {
    ...
    var timeSeries: [TimeSeries] {
        ...
        return [TimeSeries(name: "Right Ankle Circumference", data: data)]
    }
    ...
    func onAppear() async {
        entries = Self.rightAnkleEntries(from: interactor.bodyMeasurements, unit: interactor.lengthUnitPreference)
    }

    func onAddPressed() {
        router.showLogMeasurementView(kind: .rightAnkle)
    }
    ...
    private static func rightAnkleEntries(from entries: [BodyMeasurementEntry], unit: LengthUnitPreference) -> [RightAnkleMeasurementEntry] {
        entries
            ...
            .compactMap { entry in
                ...
                return RightAnkleMeasurementEntry(
                    id: entry.id,
                    date: entry.date,
                    rightAnkleCircumference: UnitConversion.convertLength(rightAnkleCircumference, to: unit)
                )
            }
...
extension RightAnkleMeasurementEntry {
    static let mocks: [RightAnkleMeasurementEntry] = [
...
extension CoreRouter {
    func showRightAnkleMeasurementView(delegate: RightAnkleMeasurementDelegate, themeColor: Color? = nil) {
        router.showScreen(.sheet) { router in
            builder.rightAnkleMeasurementView(router: router, delegate: delegate, themeColor: themeColor)
        }
    }
}

extension CoreBuilder {
    func rightAnkleMeasurementView(router: AnyRouter, delegate: RightAnkleMeasurementDelegate, themeColor: Color? = nil) -> some View {
        MetricDetailView(
