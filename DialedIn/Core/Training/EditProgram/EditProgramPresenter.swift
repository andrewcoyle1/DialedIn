...
class EditProgramPresenter {
    ...
    func navToProgramGoalsView(plan: TrainingPlan) {
        ...
        router.showProgramGoalsView(delegate: ProgramGoalsDelegate(plan: plan))
    }

    func navToProgramScheduleView(plan: TrainingPlan) {
        interactor.trackEvent(event: Event.navigate)
        router.showProgramScheduleView(delegate: ProgramScheduleDelegate(plan: plan))
