                    ...
                    Section {
                        ForEach(viewModel.scheduledWorkouts) { workout in
                            if workout.isCompleted {
                                ...
                                builder.todaysWorkoutCardView(scheduledWorkout: workout, onStart: {
                                    Task {
                                        dismiss()
...
#Preview {
    ...
    builder.dayScheduleSheetView(date: Date(), scheduledWorkouts: ScheduledWorkout.mocksWeek1, onStartWorkout: { _ in })
    .previewEnvironment()
}

#Preview("No Workouts Scheduled") {
    let builder = CoreBuilder(container: DevPreview.shared.container)
    builder.dayScheduleSheetView(date: Date(), scheduledWorkouts: [], onStartWorkout: { _ in })
    .previewEnvironment()
}
