//
//  ProgramStartConfigPresenter.swift
//  DialedIn
//
...
@Observable
@MainActor
class ProgramStartConfigPresenter {
    ...
    func navToProgramPreviewView(template: ProgramTemplateModel) {
        ...
        router.showProgramPreviewView(delegate: ProgramPreviewDelegate(template: template, startDate: startDate))
    }
    
    func onStart(onStart: (Date, Date?, String?) -> Void) {
        defer {
