//
//  OnboardingExerciseFrequencyPresenter.swift
//  DialedIn
//
...
@Observable
@MainActor
class OnboardingExerciseFrequencyPresenter {
    ...
    func navigateToOnboardingActivity(userBuilder: UserModelBuilder) {
        if let frequency = selectedFrequency {
            ...
            router.showOnboardingActivityView(delegate: OnboardingActivityDelegate(userModelBuilder: builder))
        }
    }
