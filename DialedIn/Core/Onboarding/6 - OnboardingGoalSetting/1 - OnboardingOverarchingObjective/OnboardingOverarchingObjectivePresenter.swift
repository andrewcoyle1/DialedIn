//
//  OnboardingOverarchingObjectivePresenter.swift
//  DialedIn
//
...
@Observable
@MainActor
class OnboardingOverarchingObjectivePresenter {
    ...
    func navigateToNextStep() {
        ...
        if objective == .maintain {
            ...
            router.showOnboardingGoalSummaryView(delegate: OnboardingGoalSummaryDelegate(weightGoalBuilder: weightGoalBuilder))
        } else {
            let weightGoalBuilder = WeightGoalBuilder(objective: objective)
            interactor.trackEvent(event: Event.navigate)
            router.showOnboardingTargetWeightView(delegate: OnboardingTargetWeightDelegate(weightGoalBuilder: weightGoalBuilder))
        }
    }
