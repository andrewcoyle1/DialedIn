//
//  LogLeftWristMeasurementView.swift
//  DialedIn
//
...
struct LogLeftWristMeasurementView: View {

    @State var presenter: LogLeftWristMeasurementPresenter
    ...
    var body: some View {
        ...
        .navigationTitle("Log Left Wrist Measurement")
    ...
    private var measurementPickerSection: some View {
        Section {
            if presenter.unit == .centimeters {
                Picker("Left Wrist Circumference", selection: $presenter.selectedCentimeters) {
                ...
                Picker("Left Wrist Circumference", selection: $presenter.selectedInches) {
...
extension CoreBuilder {
    func logLeftWristMeasurementView(router: AnyRouter) -> some View {
        LogLeftWristMeasurementView(
            presenter: LogLeftWristMeasurementPresenter(interactor: interactor, router: CoreRouter(router: router, builder: self))
        )
    }
}
...
extension CoreRouter {
    func showLogLeftWristMeasurementView() {
        router.showScreen(.sheetConfig(config: ResizableSheetConfig(detents: [.fraction(0.5)]))) { router in
            builder.logLeftWristMeasurementView(router: router)
        }
    }
}
