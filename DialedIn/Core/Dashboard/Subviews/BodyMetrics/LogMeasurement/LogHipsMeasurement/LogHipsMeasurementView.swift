...
struct LogHipsMeasurementView: View {

    @State var presenter: LogHipsMeasurementPresenter
    ...
    var body: some View {
        ...
        .navigationTitle("Log Hips Measurement")
    ...
    private var measurementPickerSection: some View {
        Section {
            if presenter.unit == .centimeters {
                Picker("Hips Circumference", selection: $presenter.selectedCentimeters) {
                    ForEach((80...130).reversed(), id: \.self) { value in
                ...
                Picker("Hips Circumference", selection: $presenter.selectedInches) {
                    ForEach((32...52).reversed(), id: \.self) { value in
...
extension CoreBuilder {
    func logHipsMeasurementView(router: AnyRouter) -> some View {
        LogHipsMeasurementView(
            presenter: LogHipsMeasurementPresenter(interactor: interactor, router: CoreRouter(router: router, builder: self))
        )
    }
}
...
extension CoreRouter {
    func showLogHipsMeasurementView() {
        router.showScreen(.sheetConfig(config: ResizableSheetConfig(detents: [.fraction(0.5)]))) { router in
            builder.logHipsMeasurementView(router: router)
        }
    }
}
