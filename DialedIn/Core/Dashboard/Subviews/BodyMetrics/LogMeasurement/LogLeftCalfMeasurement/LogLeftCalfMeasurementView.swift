//
//  LogLeftCalfMeasurementView.swift
//  DialedIn
//
...
struct LogLeftCalfMeasurementView: View {

    @State var presenter: LogLeftCalfMeasurementPresenter
    ...
    var body: some View {
        ...
        .navigationTitle("Log Left Calf Measurement")
    ...
    private var measurementPickerSection: some View {
        Section {
            if presenter.unit == .centimeters {
                Picker("Left Calf Circumference", selection: $presenter.selectedCentimeters) {
                ...
                Picker("Left Calf Circumference", selection: $presenter.selectedInches) {
...
extension CoreBuilder {
    func logLeftCalfMeasurementView(router: AnyRouter) -> some View {
        LogLeftCalfMeasurementView(
            presenter: LogLeftCalfMeasurementPresenter(interactor: interactor, router: CoreRouter(router: router, builder: self))
        )
    }
}
...
extension CoreRouter {
    func showLogLeftCalfMeasurementView() {
        router.showScreen(.sheetConfig(config: ResizableSheetConfig(detents: [.fraction(0.5)]))) { router in
            builder.logLeftCalfMeasurementView(router: router)
        }
    }
}
