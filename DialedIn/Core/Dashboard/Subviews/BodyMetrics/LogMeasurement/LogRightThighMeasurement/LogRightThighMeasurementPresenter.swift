//
//  LogRightThighMeasurementPresenter.swift
//  DialedIn
//
...
@Observable
@MainActor
class LogRightThighMeasurementPresenter {
    private let interactor: LogRightThighMeasurementInteractor
    private let router: LogRightThighMeasurementRouter
    ...
    init(
        interactor: LogRightThighMeasurementInteractor,
        router: LogRightThighMeasurementRouter
    ) {
    ...
    func loadInitialData() async {
        ...
        if let latest = interactor.measurementHistory
            .filter({ $0.deletedAt == nil && $0.rightThighCircumference != nil })
            .sorted(by: { $0.date > $1.date })
            .first,
           let rightThighCircumference = latest.rightThighCircumference {
            selectedCentimeters = Int(rightThighCircumference)
            selectedInches = Int(rightThighCircumference / 2.54)
        }
    ...
    func saveMeasurement() async {
        ...
        do {
            ...
            if let existingEntry = existingEntries.first {
                let updatedEntry = existingEntry.withUpdated(.rightThigh(measurementCm))
                try await interactor.updateWeightEntry(entry: updatedEntry)
            } else {
                let entry = BodyMeasurementEntry(
                    authorId: user.userId,
                    weightKg: nil,
                    rightThighCircumference: measurementCm,
                    date: selectedDate
                )
