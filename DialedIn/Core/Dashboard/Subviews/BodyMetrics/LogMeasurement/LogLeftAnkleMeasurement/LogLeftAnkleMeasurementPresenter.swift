//
//  LogLeftAnkleMeasurementPresenter.swift
//  DialedIn
//
...
@Observable
@MainActor
class LogLeftAnkleMeasurementPresenter {
    private let interactor: LogLeftAnkleMeasurementInteractor
    private let router: LogLeftAnkleMeasurementRouter
    ...
    init(
        interactor: LogLeftAnkleMeasurementInteractor,
        router: LogLeftAnkleMeasurementRouter
    ) {
    ...
    func loadInitialData() async {
        ...
        if let latest = interactor.measurementHistory
            .filter({ $0.deletedAt == nil && $0.leftAnkleCircumference != nil })
            .sorted(by: { $0.date > $1.date })
            .first,
           let leftAnkleCircumference = latest.leftAnkleCircumference {
            selectedCentimeters = Int(leftAnkleCircumference)
            selectedInches = Int(leftAnkleCircumference / 2.54)
        }
    ...
    func saveMeasurement() async {
        ...
        do {
            ...
            if let existingEntry = existingEntries.first {
                let updatedEntry = existingEntry.withUpdated(.leftAnkle(measurementCm))
                try await interactor.updateWeightEntry(entry: updatedEntry)
            } else {
                let entry = BodyMeasurementEntry(
                    authorId: user.userId,
                    weightKg: nil,
                    leftAnkleCircumference: measurementCm,
                    date: selectedDate
                )
