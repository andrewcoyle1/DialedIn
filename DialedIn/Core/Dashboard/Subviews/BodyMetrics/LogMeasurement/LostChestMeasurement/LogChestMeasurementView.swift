...
struct LogChestMeasurementView: View {

    @State var presenter: LogChestMeasurementPresenter
    ...
    var body: some View {
        ...
        .navigationTitle("Log Chest Measurement")
    ...
    private var measurementPickerSection: some View {
        Section {
            if presenter.unit == .centimeters {
                Picker("Chest Circumference", selection: $presenter.selectedCentimeters) {
                    ForEach((80...140).reversed(), id: \.self) { value in
                ...
                Picker("Chest Circumference", selection: $presenter.selectedInches) {
                    ForEach((32...56).reversed(), id: \.self) { value in
...
extension CoreBuilder {
    func logChestMeasurementView(router: AnyRouter) -> some View {
        LogChestMeasurementView(
            presenter: LogChestMeasurementPresenter(interactor: interactor, router: CoreRouter(router: router, builder: self))
        )
    }
}
...
extension CoreRouter {
    func showLogChestMeasurementView() {
        router.showScreen(.sheetConfig(config: ResizableSheetConfig(detents: [.fraction(0.5)]))) { router in
            builder.logChestMeasurementView(router: router)
        }
    }
}
