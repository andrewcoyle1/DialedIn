//
//  LogShouldersMeasurementPresenter.swift
//  DialedIn
//
...
@Observable
@MainActor
class LogShouldersMeasurementPresenter {
    private let interactor: LogShouldersMeasurementInteractor
    private let router: LogShouldersMeasurementRouter

    var selectedDate = Date()
    var selectedCentimeters: Int = 115
    var selectedInches: Int = 45
    ...
    init(
        interactor: LogShouldersMeasurementInteractor,
        router: LogShouldersMeasurementRouter
    ) {
    ...
    func loadInitialData() async {
        ...
        if let latest = interactor.measurementHistory
            .filter({ $0.deletedAt == nil && $0.shoulderCircumference != nil })
            .sorted(by: { $0.date > $1.date })
            .first,
           let shoulderCircumference = latest.shoulderCircumference {
            selectedCentimeters = Int(shoulderCircumference)
            selectedInches = Int(shoulderCircumference / 2.54)
        }
    ...
    func saveMeasurement() async {
        ...
        do {
            ...
            if let existingEntry = existingEntries.first {
                let updatedEntry = existingEntry.withUpdated(.shoulder(measurementCm))
                try await interactor.updateWeightEntry(entry: updatedEntry)
            } else {
                let entry = BodyMeasurementEntry(
                    authorId: user.userId,
                    weightKg: nil,
                    shoulderCircumference: measurementCm,
                    date: selectedDate
                )
