//
//  LogRightBicepMeasurementPresenter.swift
//  DialedIn
//
...
@Observable
@MainActor
class LogRightBicepMeasurementPresenter {
    private let interactor: LogRightBicepMeasurementInteractor
    private let router: LogRightBicepMeasurementRouter

    var selectedDate = Date()
    var selectedCentimeters: Int = 35
    var selectedInches: Int = 14
    ...
    init(
        interactor: LogRightBicepMeasurementInteractor,
        router: LogRightBicepMeasurementRouter
    ) {
    ...
    func loadInitialData() async {
        ...
        if let latest = interactor.measurementHistory
            .filter({ $0.deletedAt == nil && $0.rightBicepCircumference != nil })
            .sorted(by: { $0.date > $1.date })
            .first,
           let rightBicepCircumference = latest.rightBicepCircumference {
            selectedCentimeters = Int(rightBicepCircumference)
            selectedInches = Int(rightBicepCircumference / 2.54)
        }
    ...
    func saveMeasurement() async {
        ...
        do {
            ...
            if let existingEntry = existingEntries.first {
                let updatedEntry = existingEntry.withUpdated(.rightBicep(measurementCm))
                try await interactor.updateWeightEntry(entry: updatedEntry)
            } else {
                let entry = BodyMeasurementEntry(
                    authorId: user.userId,
                    weightKg: nil,
                    rightBicepCircumference: measurementCm,
                    date: selectedDate
                )
