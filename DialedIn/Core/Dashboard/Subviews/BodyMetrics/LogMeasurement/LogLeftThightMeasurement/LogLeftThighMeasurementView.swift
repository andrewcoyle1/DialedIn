//
//  LogLeftThighMeasurementView.swift
//  DialedIn
//
...
struct LogLeftThighMeasurementView: View {

    @State var presenter: LogLeftThighMeasurementPresenter
    ...
    var body: some View {
        ...
        .navigationTitle("Log Left Thigh Measurement")
    ...
    private var measurementPickerSection: some View {
        Section {
            if presenter.unit == .centimeters {
                Picker("Left Thigh Circumference", selection: $presenter.selectedCentimeters) {
                ...
                Picker("Left Thigh Circumference", selection: $presenter.selectedInches) {
...
extension CoreBuilder {
    func logLeftThighMeasurementView(router: AnyRouter) -> some View {
        LogLeftThighMeasurementView(
            presenter: LogLeftThighMeasurementPresenter(interactor: interactor, router: CoreRouter(router: router, builder: self))
        )
    }
}
...
extension CoreRouter {
    func showLogLeftThighMeasurementView() {
        router.showScreen(.sheetConfig(config: ResizableSheetConfig(detents: [.fraction(0.5)]))) { router in
            builder.logLeftThighMeasurementView(router: router)
        }
    }
}
