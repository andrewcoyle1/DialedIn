//
//  LogRightWristMeasurementPresenter.swift
//  DialedIn
//
...
@Observable
@MainActor
class LogRightWristMeasurementPresenter {
    private let interactor: LogRightWristMeasurementInteractor
    private let router: LogRightWristMeasurementRouter
    ...
    init(
        interactor: LogRightWristMeasurementInteractor,
        router: LogRightWristMeasurementRouter
    ) {
    ...
    func loadInitialData() async {
        ...
        if let latest = interactor.measurementHistory
            .filter({ $0.deletedAt == nil && $0.rightWristCircumference != nil })
            .sorted(by: { $0.date > $1.date })
            .first,
           let rightWristCircumference = latest.rightWristCircumference {
            selectedCentimeters = Int(rightWristCircumference)
            selectedInches = Int(rightWristCircumference / 2.54)
        }
    ...
    func saveMeasurement() async {
        ...
        do {
            ...
            if let existingEntry = existingEntries.first {
                let updatedEntry = existingEntry.withUpdated(.rightWrist(measurementCm))
                try await interactor.updateWeightEntry(entry: updatedEntry)
            } else {
                let entry = BodyMeasurementEntry(
                    authorId: user.userId,
                    weightKg: nil,
                    rightWristCircumference: measurementCm,
                    date: selectedDate
                )
