//
//  LogRightCalfMeasurementView.swift
//  DialedIn
//
...
struct LogRightCalfMeasurementView: View {

    @State var presenter: LogRightCalfMeasurementPresenter
    ...
    var body: some View {
        ...
        .navigationTitle("Log Right Calf Measurement")
    ...
    private var measurementPickerSection: some View {
        Section {
            if presenter.unit == .centimeters {
                Picker("Right Calf Circumference", selection: $presenter.selectedCentimeters) {
                    ForEach((30...50).reversed(), id: \.self) { value in
                ...
                Picker("Right Calf Circumference", selection: $presenter.selectedInches) {
                    ForEach((12...20).reversed(), id: \.self) { value in
...
extension CoreBuilder {
    func logRightCalfMeasurementView(router: AnyRouter) -> some View {
        LogRightCalfMeasurementView(
            presenter: LogRightCalfMeasurementPresenter(interactor: interactor, router: CoreRouter(router: router, builder: self))
        )
    }
}
...
extension CoreRouter {
    func showLogRightCalfMeasurementView() {
        router.showScreen(.sheetConfig(config: ResizableSheetConfig(detents: [.fraction(0.5)]))) { router in
            builder.logRightCalfMeasurementView(router: router)
        }
    }
}
