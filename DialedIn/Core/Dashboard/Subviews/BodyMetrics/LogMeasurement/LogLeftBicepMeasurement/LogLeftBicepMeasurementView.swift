//
//  LogLeftBicepMeasurementView.swift
//  DialedIn
//
...
struct LogLeftBicepMeasurementView: View {

    @State var presenter: LogLeftBicepMeasurementPresenter
    ...
    var body: some View {
        ...
        .navigationTitle("Log Left Bicep Measurement")
    ...
    private var measurementPickerSection: some View {
        Section {
            if presenter.unit == .centimeters {
                Picker("Left Bicep Circumference", selection: $presenter.selectedCentimeters) {
                ...
                Picker("Left Bicep Circumference", selection: $presenter.selectedInches) {
...
extension CoreBuilder {
    func logLeftBicepMeasurementView(router: AnyRouter) -> some View {
        LogLeftBicepMeasurementView(
            presenter: LogLeftBicepMeasurementPresenter(interactor: interactor, router: CoreRouter(router: router, builder: self))
        )
    }
}
...
extension CoreRouter {
    func showLogLeftBicepMeasurementView() {
        router.showScreen(.sheetConfig(config: ResizableSheetConfig(detents: [.fraction(0.5)]))) { router in
            builder.logLeftBicepMeasurementView(router: router)
        }
    }
}
