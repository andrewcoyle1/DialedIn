//
//  LogLeftBicepMeasurementPresenter.swift
//  DialedIn
//
...
@Observable
@MainActor
class LogLeftBicepMeasurementPresenter {
    private let interactor: LogLeftBicepMeasurementInteractor
    private let router: LogLeftBicepMeasurementRouter
    ...
    init(
        interactor: LogLeftBicepMeasurementInteractor,
        router: LogLeftBicepMeasurementRouter
    ) {
    ...
    func loadInitialData() async {
        ...
        if let latest = interactor.measurementHistory
            .filter({ $0.deletedAt == nil && $0.leftBicepCircumference != nil })
            .sorted(by: { $0.date > $1.date })
            .first,
           let leftBicepCircumference = latest.leftBicepCircumference {
            selectedCentimeters = Int(leftBicepCircumference)
            selectedInches = Int(leftBicepCircumference / 2.54)
        }
    ...
    func saveMeasurement() async {
        ...
        do {
            ...
            if let existingEntry = existingEntries.first {
                let updatedEntry = existingEntry.withUpdated(.leftBicep(measurementCm))
                try await interactor.updateWeightEntry(entry: updatedEntry)
            } else {
                let entry = BodyMeasurementEntry(
                    authorId: user.userId,
                    weightKg: nil,
                    leftBicepCircumference: measurementCm,
                    date: selectedDate
                )
