//
//  LogLeftForearmMeasurementView.swift
//  DialedIn
//
...
struct LogLeftForearmMeasurementView: View {

    @State var presenter: LogLeftForearmMeasurementPresenter
    ...
    var body: some View {
        ...
        .navigationTitle("Log Left Forearm Measurement")
    ...
    private var measurementPickerSection: some View {
        Section {
            if presenter.unit == .centimeters {
                Picker("Left Forearm Circumference", selection: $presenter.selectedCentimeters) {
                ...
                Picker("Left Forearm Circumference", selection: $presenter.selectedInches) {
            ...
            Text("Left Forearm Circumference")
        }
        .removeListRowFormatting()
    }
...
extension CoreBuilder {
    func logLeftForearmMeasurementView(router: AnyRouter) -> some View {
        LogLeftForearmMeasurementView(
            presenter: LogLeftForearmMeasurementPresenter(interactor: interactor, router: CoreRouter(router: router, builder: self))
        )
    }
}
...
extension CoreRouter {
    func showLogLeftForearmMeasurementView() {
        router.showScreen(.sheetConfig(config: ResizableSheetConfig(detents: [.fraction(0.5)]))) { router in
            builder.logLeftForearmMeasurementView(router: router)
        }
    }
}
