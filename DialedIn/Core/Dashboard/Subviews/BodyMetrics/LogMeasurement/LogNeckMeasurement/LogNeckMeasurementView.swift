...
struct LogNeckMeasurementView: View {

    @State var presenter: LogNeckMeasurementPresenter
    ...
    var body: some View {
        ...
        .navigationTitle("Log Neck Measurement")
    ...
    private var measurementPickerSection: some View {
        Section {
            if presenter.unit == .centimeters {
                Picker("Neck Circumference", selection: $presenter.selectedCentimeters) {
                    ForEach((10...60).reversed(), id: \.self) { value in
                ...
                Picker("Neck Circumference", selection: $presenter.selectedInches) {
                    ForEach((4...24).reversed(), id: \.self) { value in
...
extension CoreBuilder {
    func logNeckMeasurementView(router: AnyRouter) -> some View {
        LogNeckMeasurementView(
            presenter: LogNeckMeasurementPresenter(interactor: interactor, router: CoreRouter(router: router, builder: self))
        )
    }
}
...
extension CoreRouter {
    func showLogNeckMeasurementView() {
        router.showScreen(.sheetConfig(config: ResizableSheetConfig(detents: [.fraction(0.5)]))) { router in
            builder.logNeckMeasurementView(router: router)
        }
    }
}
