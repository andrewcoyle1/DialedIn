...
struct LogBustMeasurementView: View {

    @State var presenter: LogBustMeasurementPresenter
    ...
    var body: some View {
        ...
        .navigationTitle("Log Bust Measurement")
    ...
    private var measurementPickerSection: some View {
        Section {
            if presenter.unit == .centimeters {
                Picker("Bust Circumference", selection: $presenter.selectedCentimeters) {
                    ForEach((70...130).reversed(), id: \.self) { value in
                ...
                Picker("Bust Circumference", selection: $presenter.selectedInches) {
                    ForEach((28...52).reversed(), id: \.self) { value in
...
extension CoreBuilder {
    func logBustMeasurementView(router: AnyRouter) -> some View {
        LogBustMeasurementView(
            presenter: LogBustMeasurementPresenter(interactor: interactor, router: CoreRouter(router: router, builder: self))
        )
    }
}
...
extension CoreRouter {
    func showLogBustMeasurementView() {
        router.showScreen(.sheetConfig(config: ResizableSheetConfig(detents: [.fraction(0.5)]))) { router in
            builder.logBustMeasurementView(router: router)
        }
    }
}
