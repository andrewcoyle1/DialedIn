//
//  LogBustMeasurementPresenter.swift
//  DialedIn
//
...
@Observable
@MainActor
class LogBustMeasurementPresenter {
    private let interactor: LogBustMeasurementInteractor
    private let router: LogBustMeasurementRouter
    ...
    init(
        interactor: LogBustMeasurementInteractor,
        router: LogBustMeasurementRouter
    ) {
    ...
    func loadInitialData() async {
        ...
        if let latest = interactor.measurementHistory
            .filter({ $0.deletedAt == nil && $0.bustCircumference != nil })
            .sorted(by: { $0.date > $1.date })
            .first,
           let bustCircumference = latest.bustCircumference {
            selectedCentimeters = Int(bustCircumference)
            selectedInches = Int(bustCircumference / 2.54)
        }
    ...
    func saveMeasurement() async {
        ...
        do {
            ...
            if let existingEntry = existingEntries.first {
                let updatedEntry = existingEntry.withUpdated(.bust(measurementCm))
                try await interactor.updateWeightEntry(entry: updatedEntry)
            } else {
                let entry = BodyMeasurementEntry(
                    authorId: user.userId,
                    weightKg: nil,
                    bustCircumference: measurementCm,
                    date: selectedDate
                )
