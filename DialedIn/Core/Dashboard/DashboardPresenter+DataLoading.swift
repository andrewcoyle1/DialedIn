//
//  DashboardPresenter+DataLoading.swift
//  DialedIn
//
...
extension DashboardPresenter {
    ...
    func loadLocalScaleWeightEntries() {
        scaleWeightEntries = (try? interactor.readAllLocalWeightEntries()) ?? interactor.measurementHistory
    }

    func loadRemoteScaleWeightEntriesIfNeeded() async {
        guard let userId = interactor.userId else { return }
        if let entries = try? await interactor.readAllRemoteWeightEntries(userId: userId) {
            scaleWeightEntries = entries
        }
    }
    ...
    func loadMuscleGroupsData() async {
        ...
        do {
            ...
            muscleGroupCards = [Muscles.upperBack, Muscles.rearDelts].map { muscle in
                MuscleGroupCardItem(muscle: muscle, last7DaysData: Array(repeating: 0.0, count: 7), totalSets: 0.0)
            }
        }
    }
    ...
    func loadWeighInData() {
        do {
            ...
            weighInContributionData = Array(repeating: 0.0, count: 30)
            weighInCountThisWeek = 0
        }
    }
