...
| **`supersetAutoScroll`** | `WorkoutSettings` | `WorkoutSettingsPresenter.supersetAutoScroll` | **nothing** | `wire` — `WorkoutTrackerPresenter.advanceAfterExerciseCompletion(exerciseIndex:in:)`, which already honours `exerciseAutoNext` and is the only place focus moves between exercises. Superset members are known there via `supersetGroupId`. |
