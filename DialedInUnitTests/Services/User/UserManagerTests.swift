...
struct UserManagerTests {
    ...
    ) async throws -> (UserManager, RecordingRemoteDocumentService<UserModel>) {
    ...
    func testFollowingAPublicProfileIsImmediate() async throws {
        ...
        let (manager, remote) = try await recordingManager(UserModel(userId: "me"), queryService: queries)
    ...
    func testRequestingAPrivateProfileCreatesAPendingRequestAndCancelRemovesIt() async throws {
        let queries = MockUserQueryService()
        let (manager, remote) = try await recordingManager(UserModel(userId: "me", submittedFirstName: "Sam"), queryService: queries)
    ...
    func testSigningInSeedsSentAndIncomingRequests() async throws {
        ...
        let (manager, _) = try await recordingManager(UserModel(userId: "me"), queryService: queries)
