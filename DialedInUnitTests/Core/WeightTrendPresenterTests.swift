...
struct WeightTrendPresenterTests {
    ...
    ) -> (WeightTrendPresenter, Interactor, Router) {
        let interactor = Interactor()
        interactor.bodyMeasurements = measurements
        let router = Router()
        return (WeightTrendPresenter(interactor: interactor, router: router), interactor, router)
    }
    ...
    func testATrendLineIsPlottedAlongsideTheReadings() {
        ...
        #expect(presenter.timeSeries.map(\.name) == ["Scale Weight", "Trend Weight"])
    }
    ...
    func testOneWeighInGetsNoTrendLine() {
        let (presenter, _, _) = presenter([entry(id: "e1", weightKg: 73.0, daysAgo: 0)])

        #expect(presenter.timeSeries.map(\.name) == ["Scale Weight"])
    }
    ...
    func testNoWeighInsLeaveTheChartEmpty() {
        ...
        #expect(presenter.timeSeries.first { $0.name == "Scale Weight" }?.data.isEmpty == true)
    }
    ...
    func testEntriesWithoutAWeightAreLeftOut() {
        ...
        #expect(presenter.timeSeries.first { $0.name == "Scale Weight" }?.data.count == 1)
    }
    ...
    func testTheTrendStartsAtTheFirstReading() throws {
        ...
        let first = try #require(presenter.entries.first)
    ...
    func testASpikeBarelyMovesTheTrend() throws {
        ...
        let (presenter, _, _) = presenter(steady + [entry(id: "spike", weightKg: 78.0, daysAgo: 0)])

        let last = try #require(presenter.entries.last)
    ...
    func testTheTrendHasARowPerWeighInOldestFirst() {
        ...
        #expect(presenter.entries.count == 3)
        #expect(presenter.entries.map(\.date) == presenter.entries.map(\.date).sorted())
    }
    ...
    func testTheScreenIsConfiguredAsAWeightChart() {
        ...
        #expect(presenter.configuration.title == "Weight Trend")
        #expect(presenter.supportsDeletion)
    }

    @Test("Test Adding A Weight Opens The Logging Screen")
    func testAddingAWeightOpensTheLoggingScreen() {
        let (presenter, _, router) = presenter([])

        presenter.onAddWeightPressed()

        #expect(router.didShowLogWeight)
    }
    ...
    func testAppearingAgainPicksUpANewlyLoggedWeight() async {
        let (presenter, interactor, _) = presenter([entry(id: "e1", weightKg: 73.0, daysAgo: 1)])
        #expect(presenter.entries.count == 1)

        interactor.bodyMeasurements.append(entry(id: "e2", weightKg: 72.6, daysAgo: 0))
    ...
    func testDeletingAWeighInClearsOnlyItsWeight() async throws {
        ...
        let (presenter, interactor, _) = presenter([measured])
        let row = try #require(presenter.entries.first)

        await presenter.onDeleteEntry(row)

        let saved = try #require(interactor.savedMeasurements.first)
